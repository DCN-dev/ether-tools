package dev.dcn.web3.gen;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import dev.dcn.web3.FileUtils;
...
public class JavaContractGenerator {
    ...
    public static String ABITypeConstructor(String solidityType, String arg) {
        if (solidityType.endsWith("[]")) {
            String itemType = ABIGeneratedType(solidityType.substring(0, solidityType.length() - 2));
            return "org.web3j.abi.datatypes.DynamicArray<" + itemType + ">("
                    + itemType + ".class, " +
                    arg + ".stream().map(" + itemType + "::new).collect(Collectors.toList())"
                    + ")";
        }
    ...
    private static String ToJavaType(String solidityType) {
        if (solidityType.endsWith("[]")) {
            return "List<" + ToJavaType(solidityType.substring(0, solidityType.length() - 2)) + ">";
        }
